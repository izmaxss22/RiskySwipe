...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IPointerDownHandler, IPointerUpHandler
    {
                ...
                foreach (TransitionEffect transitionEffect in transitionEffects)
                {
                    ...
                    switch (transitionEffect.Label)
                    {
                        ...
                        case "color.r":
                            ...
                            {
                                graphic.color = new UnityEngine.Color(transitionEffect.GetValue(displacement), graphic.color.g, graphic.color.b, graphic.color.a);
                            }
                            break;
                        case "color.g":
                            graphics = panel.GetComponentsInChildren<Graphic>();
                            foreach (Graphic graphic in graphics)
                            {
                                graphic.color = new UnityEngine.Color(graphic.color.r, transitionEffect.GetValue(displacement), graphic.color.b, graphic.color.a);
                            }
                            break;
                        case "color.b":
                            graphics = panel.GetComponentsInChildren<Graphic>();
                            foreach (Graphic graphic in graphics)
                            {
                                graphic.color = new UnityEngine.Color(graphic.color.r, graphic.color.g, transitionEffect.GetValue(displacement), graphic.color.a);
                            }
                            break;
                        case "color.a":
                            graphics = panel.GetComponentsInChildren<Graphic>();
                            foreach (Graphic graphic in graphics)
                            {
                                graphic.color = new UnityEngine.Color(graphic.color.r, graphic.color.g, graphic.color.b, transitionEffect.GetValue(displacement));
                            }
                            break;
                    }
                }
        ...
        public void Add_WithName(GameObject panel, int index, int levelNumberForName)
        {
            if (NumberOfPanels != 0 && (index < 0 || index > NumberOfPanels))
            {
                Debug.LogError("<b>[SimpleScrollSnap]</b> Index must be an integer from 0 to " + NumberOfPanels + ".", gameObject);
            ...
            panel = Instantiate(panel, Content, false);
            panel.name = levelNumberForName.ToString();
            panel.transform.SetSiblingIndex(index);
